'''
Duration-sweep Rabi -- Hermite pulse (Python-side sweep)
=======================================================================
...
yields a square wave. Truncating a Hermite pulse does NOT yield the same
...
requested duration it builds a self-similar Hermite envelope (the shape
...
class RabiFineRes(ReadoutHelpers, NVAveragerProgram):
    ...
    def initialize(self):
        ...
        data[:self.cfg.mw_pulse_ftsamp] = hermite_envelope(self.cfg.mw_pulse_ftsamp)
...
def sweep_rabi_duration(config, durations_ftns, progress=True):
    ...
    iterator = tqdm(durations_ftns, disable=not progress, desc="hermite duration Rabi")
