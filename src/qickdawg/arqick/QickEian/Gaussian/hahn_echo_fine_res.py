'''
Subnanosecond Hahn Echo -- Gaussian pulse
=======================================================================
pi/2_X - tau - pi_X - tau - pi/2_X with fine timed delays.

Ported from the square-pulse hahn_echo_fine_res.py with Gaussian pi and
pi/2 envelopes at each sub-clock offset. One latent bug in the square
original is also fixed here (it never fired because the notebooks run
Hahn echo through CPMGXYFineRes with n_cpmg=1 instead): body() called
self.signal_and_reference_readout(...), a method that does not exist in
ReadoutHelpers -- the correct name is readout_and_reference().
'''
...
class HahnEchoFineRes(ReadoutHelpers, NVAveragerProgram):
    ...
    def initialize(self):
        ...
        pi_env = gaussian_envelope(self.cfg.mw_pi2_ftsamp * 2)
        half_pi_env = gaussian_envelope(self.cfg.mw_pi2_ftsamp)
    ...
    def body(self):
        ...
        # FIX vs square original: was self.signal_and_reference_readout(...),
        # which does not exist in ReadoutHelpers.
    ...
    def program_pulses(self, iter):
        """
        1. pi/2: index = 0
        2. pi: index <= index + tau - half_pi_len_unused
        3. pi/2: index <= index + tau - pi_len_unused
        """
        # Pi/2 X pulse
