'''
Ramsey sub-nanosecond resolution pulsing program. 2nd pulse has no gain.
Pi pulse used.
=======================================================================
...
class PiRamseyFineRes(NVAveragerProgram):
    '''
    Ramsey sub-nanosecond resolution pulsing program. 2nd pulse has no gain.
    Pi pulse used.
    '''
    required_cfg = [        
        "mw_pi_tdds", # length of pi pulse
        "delay_tdds_start",
        "delay_tdds_end",
        "nsweep_points",
        ...
        "after_pump_time_tdds",
    ]

    def initialize(self):
        self.check_cfg()
        if self.cfg.first_pulse_mw_gain == None:
            self.cfg.first_pulse_mw_gain = self.cfg.mw_gain
        # Get mw registers
        ...
        self.pi_waveform_len_treg = max(int(np.ceil((self.cfg.mw_pi_tdds + self.samps_per_clk-1) / self.samps_per_clk)), 3)
        self.pi_to_pi_correction_tdds = self.cfg.mw_pi_tdds

        for i in range(16):
            # pi pulse
            i_data = np.zeros(self.pi_waveform_len_treg * self.samps_per_clk)
            q_data = np.zeros(self.pi_waveform_len_treg * self.samps_per_clk)
            i_data[i : i + self.cfg.mw_pi_tdds] = 1
            q_data[i : i + self.cfg.mw_pi_tdds] = 1
        ...
        self.pi_len_unused_tdds = self.pi_waveform_len_treg*self.samps_per_clk - self.cfg.mw_pi_tdds
    ...
    def body(self):
        ...
        self.set_pulse_registers(ch=self.cfg.mw_channel, waveform="pi_0", gain = self.cfg.first_pulse_mw_gain, phase=self.deg2reg(0))
        self.pulse(ch=self.cfg.mw_channel)
        self.sync_all()

        self.set_pulse_registers(ch=self.cfg.mw_channel, waveform="pi_0", gain = 0, phase=self.deg2reg(180))
    ...
    def offset_computations(self):
        ...
        self.tdds_offset_register.set_to(self.tdds_offset_register, '+', self.cfg.after_pump_time_tdds)
        ...
        self.address_register.set_to(self.tdds_offset_register, '*', self.pi_waveform_len_treg, physical_unit = False)