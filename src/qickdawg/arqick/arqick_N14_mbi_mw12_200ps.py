'''
N14 Mbi, Pass = N, Repeats MW1 + MW2 + N14 SR
'''
...
class N14MBIMW12RepeatsAnyNFineRes(StandardOps, NVAveragerProgram):
    ...
    required_cfg = [
        ...
        "pmod_out_trig_to_mw12_repeats_treg",
    ...
    def body(self):
        ...
        self.sync_all(self.cfg.pmod_out_trig_to_mw12_repeats_treg)
        ##

        self.label("LOOP_n_mcond")
        
        #### PMOD trigger to TTL2, fire out MW1 again
        # electron pi pulse
        self.set_pulse_registers(ch=self.cfg.mw_channel, waveform="pi_0", freq=self.cfg.freq_freg, gain=self.cfg.mw_gain, phase=self.deg2reg(0))
        self.pulse(ch=self.cfg.mw_channel)
        self.sync_all()
