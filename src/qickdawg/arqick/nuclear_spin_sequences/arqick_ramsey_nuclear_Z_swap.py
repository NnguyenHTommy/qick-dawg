'''
Ramsey nuclear Z with reduced swap in beginning
'''
...
class RamseyNuclearZSwap(StandardOps, NVAveragerProgram):
    ...
    def body(self):
        ...
        # n RX(pi/2)
        self.tdds_offset_register.set_to(self.tdds_offset_register, '+', self.cfg.delay_tdds_gate_crxpi2 - self.cfg.delay_tdds_gate_urxpi2)
        self.cpmg_xy8_gate(gate_index=4, pi_2_pulse_before=False, delay_tau_tdds=self.cfg.delay_tdds_gate_urxpi2, n_cpmg_pulses=self.cfg.n_cpmg_gate_urxpi2)

       # e RY(pi/2)
        self.set_pulse_registers(ch=self.cfg.mw_channel, waveform="half_pi_0", freq=self.cfg.freq_freg, gain=self.cfg.mw_gain, phase=self.deg2reg(90))
        self.offset_computations(pi2_after=True, delay_tau_tdds=self.cfg.delay_tdds_gate_urxpi2)
        self.sync(self.treg_offset_register.page, self.treg_offset_register.addr)
        self.pulse(ch=self.cfg.mw_channel)
        self.sync_all()

        # n RZ(pi/2)
        self.cpmg_xy8_gate(gate_index=5, pi_2_pulse_before=True, delay_tau_tdds=self.cfg.delay_tdds_gate_rzpi2, n_cpmg_pulses=self.cfg.n_cpmg_gate_rzpi2)

        # e-n CROTX(pi/2)
        self.tdds_offset_register.set_to(self.tdds_offset_register, '+', self.cfg.delay_tdds_gate_rzpi2 - self.cfg.delay_tdds_gate_crxpi2)
        self.cpmg_xy8_gate(gate_index=6, pi_2_pulse_before=False, delay_tau_tdds=self.cfg.delay_tdds_gate_crxpi2, n_cpmg_pulses=self.cfg.n_cpmg_gate_crxpi2)
