'''
Unconditional rotation sweep N on ms = -1
'''
...
class UnConditionalRotationMS1(StandardOps, NVAveragerProgram):
    ...
    def body(self):
        ...
        self.tdds_offset_register.set_to(self.tdds_offset_register, '-', self.cfg.delay_tdds_gate_urxpi2) 
        # e-n CROTX(pi/2) with vary N
        self.cpmg_xy8_gate(gate_index=3, pi_2_pulse_before=False, delay_tau_tdds=self.cfg.delay_tdds_gate_urxpi2, n_cpmg_pulses=None, vary_n=True)
        
        # e RX(pi)
        self.set_pulse_registers(ch=self.cfg.mw_channel, waveform="pi_0", freq=self.cfg.freq_freg, gain=self.cfg.mw_gain, phase=self.deg2reg(0))
        self.tdds_offset_register.set_to(self.tdds_offset_register, '+', self.cfg.delay_tdds_gate_urxpi2 - self.pi_to_pi_correction_tdds - self.pi_len_unused_tdds)
        # Coarse delay in treg units.
        self.bitwi(
            self.tdds_offset_register.page,
            self.treg_offset_register.addr,
