'''
PulsePol - Ramsey sub-nanosecond resolution pulsing program
=======================================================================
Min resolution of 200ps for delay steps between pulses in PulsePol - Ramsey sequence
using fine control of waveform start address and phase.
Precession time in PulsePol is fixed, while it will be swept in Ramsey.
Sequence:   [(pi/2_X - tau_fixed - pi_Y - tau_fixed - pi/2_X - pi/2_Y - tau_fixed - pi_X - tau_fixed - -pi/2_Y) x 2Npol]
          - [(pi/2_X - tau - pi/2_Y)]
'''
...
class PulsePolRamseyFineRes(NVAveragerProgram):
    '''
    PulsePol - Ramsey sub-nanosecond resolution pulsing program
    '''
    required_cfg = [
        "mw_pi2_tdds",  # length of pi/2 pulse
        "delay_tdds_start",  # for ramsey
        "delay_tdds_end",  # for ramsey
        "nsweep_points",  # for ramsey
        "delay_tdds_fixed",  # fixed delay for pulsepol
        "freq_freg",  # Microwave freq
        "freq_freg_detuned",  # Detuned Microwave freq for ramsey
    ...
    def initialize(self):
        ...
        # declare the mw channel output on the board (its dac0)
        ...
        self.pi_xy_waveform_len_treg = max(
            int(np.ceil((self.pi_len_tdds + self.samps_per_clk - 1) / self.samps_per_clk)), 3)
        self.pi_waveform_len_treg = max(int(np.ceil((self.pi_len_tdds + self.samps_per_clk - 1) / self.samps_per_clk)),
                                        3)
        ...
        self.n_pulsepol_register = self.new_gen_reg(self.cfg.mw_channel,
                                                    name='n_pulsepol',
                                                    init_val=(2 * self.cfg.n_pulsepol) - 1)
        ...
        self.delay_register = self.new_gen_reg(self.cfg.mw_channel,
                                               name='delay',
                                               init_val=self.cfg.delay_tdds_start)  # initialization happens when adding sweep

        # for pulsepol
        self.delay_fixed_register = self.new_gen_reg(self.cfg.mw_channel,
                                                     name='delay_fixed',
                                                     init_val=self.cfg.delay_tdds_fixed)
    ...
    def body(self):
        ...
        # NOTE: this instruction should always jump, since register 0 == register 0.
        ...
        # 9 Ramsey start: π/2_x
        self.address_register.set_to(self.tdds_offset_register, '*', self.half_pi_waveform_len_treg + self.pi_waveform_len_treg + self.pi_xy_waveform_len_treg + self.pi_xy_waveform_len_treg, physical_unit=False)
        self.set_pulse_registers(ch=self.cfg.mw_channel,
                                 waveform="half_pi_0",
                                 freq=self.cfg.freq_freg_detuned,
                                 phase=self.deg2reg(0))
        self.pulse(ch=self.cfg.mw_channel)
        self.sync_all()
    ...
    def program_pulse(self, type="pi", phase=0):
        ...
        self.tdds_offset_register.set_to(self.tdds_offset_register, '+', self.delay_fixed_register)
        ...
        self.set_pulse_registers(ch=self.cfg.mw_channel, waveform=f"{type}_0", freq=self.cfg.freq_freg, phase=self.deg2reg(phase))
        ...
        if type == "pi":
            ...
            self.address_register.set_to(self.address_register, '+', self.half_pi_waveform_len_treg + self.pi_waveform_len_treg, physical_unit=False)
        elif type == "half_pi_yx":
            self.address_register.set_to(self.address_register, '+', self.half_pi_waveform_len_treg + self.pi_waveform_len_treg + self.pi_xy_waveform_len_treg, physical_unit=False)
