'''
Bootstrap sub-nanosecond resolution pulsing program -- Hermite pulse
=======================================================================
...
of the composite pulse_seq envelope is filled with a self-similar Hermite
shape (stretched to that segment's own length) instead of a flat block, so
every sub-pulse in the sequence gets a proper Hermite envelope.
'''
...
class BootstrapFineRes(NVAveragerProgram):
    '''
    Bootstrap sub-nanosecond resolution pulsing program -- Hermite pulse shape.
    '''
    required_cfg = [        
