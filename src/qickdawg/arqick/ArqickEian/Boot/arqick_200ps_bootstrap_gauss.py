'''
Bootstrap sub-nanosecond resolution pulsing program -- Gaussian pulse
=======================================================================
...
Same protocol as the square-pulse BootstrapFineRes -- each pi2/pi segment
of the composite pulse_seq envelope is filled with a self-similar Gaussian
shape (stretched to that segment's own length) instead of a flat block, so
every sub-pulse in the sequence gets a proper Gaussian envelope.
'''
...
class BootstrapFineRes(NVAveragerProgram):
    '''
    Bootstrap sub-nanosecond resolution pulsing program -- Gaussian pulse shape.
    '''
    required_cfg = [        
    ...
    def make_pulse_envelope(self):
        ...
        for typ, ax in seqs[self.cfg.bootstrap_experiment_number]:
            ...
            i_val, q_val = axis[ax]
            env = gaussian_envelope(length)
            i_data[idx : idx + length] = i_val * env
            q_data[idx : idx + length] = q_val * env
            idx += length
