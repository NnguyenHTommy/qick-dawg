'''
Duration-sweep Rabi -- Sinc pulse -- ARQICK (ARTIQ+QICK) variant
=======================================================================
Same reasoning as the QICK-only version: truncating a sinc envelope
doesn't yield a shorter sinc pulse, so duration can't be swept on-chip
the way square's periodic-replay trick does. Duration is still swept
point-by-point, but the loop now lives on the ARTIQ side (one
...
class RabiFineRes(NVAveragerProgram):
    ...
    def initialize(self):
        ...
        data[:self.cfg.mw_pulse_tdds] = sinc_envelope(self.cfg.mw_pulse_tdds)
