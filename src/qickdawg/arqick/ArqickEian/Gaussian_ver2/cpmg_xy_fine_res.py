'''
CPMG-XY Fine-Resolution Pulse Sequence -- Gaussian pulse -- ARQICK variant
=======================================================================
Same XY8 phase-cycled CPMG machinery as the QICK-only Gaussian
...
only change is the readout mechanism.
...
class CPMGXYFineRes(NVAveragerProgram):
    ...
    def initialize(self):
        """
        Prepare Gaussian pi/pi2 waveforms (one per sub-clock offset), FPGA
        ...
        pi_env = gaussian_envelope(self.cfg.mw_pi2_tdds * 2)
        half_pi_env = gaussian_envelope(self.cfg.mw_pi2_tdds)
        ...
        # Tau is now center-to-center between pulses (matches qickdawg.arqick.StandardOps),
        # not window-edge-to-window-edge like the original square/hermite CPMG files: each
        # correction below removes the half-widths of the two pulses bounding a gap, plus
        # the trailing pulse's buffer deadtime; corr_loop_preload absorbs the mismatch for
        # the first (half_pi->pi) gap, whose buffer deadtime differs from steady-state loop gaps.
        pi_to_pi2_phys = self.cfg.mw_pi2_tdds + self.cfg.mw_pi2_tdds // 2
        pi_to_pi_phys = self.cfg.mw_pi2_tdds * 2
        pi2_to_pi2_phys = self.cfg.mw_pi2_tdds
        self.corr_loop = pi_to_pi_phys + self.pi_len_unused
        self.corr_close = pi_to_pi2_phys + self.pi_len_unused
        self.corr_ramsey = pi2_to_pi2_phys + self.half_pi_len_unused
        self.corr_loop_preload = self.corr_loop - (pi_to_pi2_phys + self.half_pi_len_unused)
    ...
    def program_pulses(self, label_id):
        ...
        if self.cfg.n_cpmg > 0:
            ...
            # Subtract tau so the first inter-pulse delay is tau (not 2tau), then apply
            # corr_loop_preload so this first (half_pi->pi) gap gets the right correction
            # even though the loop body below always applies the steady-state pi->pi one.
            ...
            self.mathi(self.delay_fine_samples.page, self.delay_fine_samples.addr,
                        self.delay_fine_samples.addr, "+", self.corr_loop_preload)
    ...
    def set_waveform(self, pi_pulse=True, phase=None):
        if pi_pulse:
            correction = self.corr_loop
        else:
            correction = self.corr_close if self.cfg.n_cpmg > 0 else self.corr_ramsey
        self.offset_computations(double_tau=pi_pulse, correction_tdds=correction)
    ...
    def offset_computations(self, double_tau=False, correction_tdds=0):
        self.math(self.delay_fine_samples.page, self.delay_fine_samples.addr,
                  self.delay_fine_samples.addr, "+", self.tau.addr)
        if double_tau:
            self.math(self.delay_fine_samples.page, self.delay_fine_samples.addr,
                      self.delay_fine_samples.addr, "+", self.tau.addr)
        self.mathi(self.delay_fine_samples.page, self.delay_fine_samples.addr,
                    self.delay_fine_samples.addr, "-", correction_tdds)
